...
use crate::social_network::state::{
    VerticesSelected, VerticesSelectedResult, VerticesSetError, VerticesSetSuccess,
};
use std::sync::Mutex;
...
pub fn get_vertices_target(app: &AppHandle) -> Result<VerticesSelected, VerticesSetError> {
    ...
    if state.vertex_1.is_empty() || state.vertex_2.is_empty() {
        ...
        return Err(VerticesSetError {
            response_code: 401,
            message:
                "No target vertex or graph type. Please choose at SNA > File > Locate Vertices"
                    .to_string(),
        });
    }
    if state.graph_type.is_empty() {
        ...
        return Err(VerticesSetError {
            response_code: 401,
            message: "No graph type. Please choose at SNA > File > Locate Vertices".to_string(),
        });
    }
    Ok(state)
}
...
pub fn set_vertices(app: AppHandle, vertices_selected: VerticesSelected) -> VerticesSelectedResult {
    ...
    if vertex_choosed.vertex_1.is_empty()
        || vertex_choosed.vertex_2.is_empty()
        || vertex_choosed.graph_type.is_empty()
    {
        log::warn!("[SNA304] No vertex target");
...
fn save_vertices_to_database(
    app: &AppHandle,
    vertices: &VerticesSelected,
    current_timestamp: String,
) -> VerticesSelectedResult {
    ...
    match db_result {
        DbConnectionProcess::Success(db_success) => {
            ...
            log::info!(
                "[SNA200] Success connect {} {}",
                response_code,
                db_success.message.unwrap_or_default()
            );
